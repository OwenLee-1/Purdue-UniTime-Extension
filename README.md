...
A Chrome extension that shows **RateMyProfessors** ratings and **average course GPA**
right next to professor names on Purdue's **UniTime** scheduling site
(`timetable.mypurdue.purdue.edu`), so you can compare professors without leaving the page.

> Status: **alpha**. Core features below are working. See [`BUILD_PLAN.md`](./BUILD_PLAN.md)
> for the roadmap.

## What it does

- **Inline rating pill** next to each instructor: `★ 4.3 · 3.2 GPA` (RateMyProfessors
  overall rating + that professor's average GPA for the course).
- **Hover card** with the full picture: difficulty, would-take-again %, top tags,
  number of ratings, and a link to the RateMyProfessors profile.
- **Best-section highlight**: when a course has multiple sections, the highest-rated
  professor is flagged so you can pick at a glance.
- **Average GPA** comes from the open [BoilerGrades](https://www.boilergrades.com/)
  ...
  it works offline.
- **Hide UniTime's hover schedule preview** (optional toggle) to reduce clutter.
...
## Installing (alpha testers)

You don't need Node or any build tools — just the packaged extension:
