...
async function processCsv(path, acc) {
  ...
  for (let r = 1; r < lines.length; r++) {
    ...
    acc[courseKey][nameKey] = acc[courseKey][nameKey] || { sum: 0, n: 0 };
    acc[courseKey][nameKey].sum += gpa;
    acc[courseKey][nameKey].n += 1;
  }
}
...
async function main() {
  ...
  // Collapse to { course: { lastname: gpaRounded } }, plus a sample count.
  const out = {};
  let entries = 0;
  for (const [course, profs] of Object.entries(acc)) {
    out[course] = {};
    for (const [name, { sum, n }] of Object.entries(profs)) {
      out[course][name] = { gpa: Math.round((sum / n) * 100) / 100, n };
      entries++;
    }
  }
  ...
  const banner =
    ...
    '// Average GPA per "SUBJECT NUMBER" course, keyed by instructor last name.\n' +
